...
// void PrintPossibleBaudrate()
// {
// 	bool hasFailed = VCS_GetBaudrateSelection(char* DeviceName, char* ProtocolStackName, char*
...
// DWORD errorCode = 0;
// //get first baudrate
// if (VCS_GetBaudrateSelection (deviceName, protocolStackName, interfaceName, portName, TRUE, &baudrateSel, &endOfSelection, &errorCode))
// 	{
// 		//get next baudrate (as long as endOfSelection == FALSE)
// 		while (!endOfSelection)
// 		{
// 			VCS_GetBaudrateSelection (deviceName, protocolStackName, interfaceName, portName, FALSE, &baudrateSel, &endOfSelection, &errorCode);
// 			std::cout<<baudrateSel<<" bit/s"<<std::endl;
// 		}
// 	}
// }
...
bool RunVelocityMode(HANDLE p_DeviceHandle, unsigned short p_usNodeId, unsigned int & p_rlErrorCode)
{
	int lResult = MMC_SUCCESS;
	DevicePort dev (p_DeviceHandle, p_usNodeId, &p_rlErrorCode, &lResult);
		// We could use VCS_SetVelocityProfile(...) to set the acceleration and deceleration; by default it is unconstrained; see Epos_parameter.dcf file in the wizard to set it in a GUI, or use VCS_GetVelocityProfile(...)
	if(VCS_ActivateProfileVelocityMode(p_DeviceHandle, p_usNodeId, &p_rlErrorCode) == 0)
	{
		LogError("VCS_ActivateProfileVelocityMode", lResult, p_rlErrorCode);
		lResult = MMC_FAILED;
...
int Demo(unsigned int* p_pErrorCode)
{
	...
	lResult = RunVelocityMode(g_pKeyHandle, g_usNodeId, lErrorCode);

	if(lResult != MMC_SUCCESS)
	{
		LogError("RunVelocityMode", lResult, lErrorCode);
	}
	// else
	// {
